...
async def measure_latency(client, method, *args, iterations=PERF_TEST_ITERATIONS, **kwargs):
    ...
    for _ in range(iterations):
        ...
        result = await client.call("perf_server", method, *args, **kwargs)
...
async def main():
    ...
    for size in [10, 100, 1000, 10000, 100000]:
        ...
        print(f"Payload {size:6d}B: {stats['mean']:6.2f}ms avg "
    ...
    for shape in [(10, 10), (100, 100), (200, 200)]:  # 200x200 instead of 500x500 to avoid max payload
        data = np.random.rand(*shape)
        stats = await measure_latency(client, "echo", data, iterations=20)
        size_kb = data.nbytes / 1024
        print(f"Array {shape[0]:3d}x{shape[1]:3d} ({size_kb:.1f}KB): "
    ...
    for n in [1000, 10000, 100000]:
        ...
        print(f"CPU task n={n:6d}: {stats['mean']:6.2f}ms avg "
              f"(min:{stats['min']:.2f} max:{stats['max']:.2f})")
    ...
    for size in [100, 500, 1000]:
        ...
        print(f"Memory task {size:4d}x{size:4d}: {stats['mean']:6.2f}ms avg "
              f"(min:{stats['min']:.2f} max:{stats['max']:.2f})")
    ...
    print(f"Minimal payload (int): {stats['mean']:6.2f}ms avg "
    ...
    for concurrency in [1, 10, 50, 100]:
        ...
        print(f"Concurrency {concurrency:3d}: {elapsed:6.2f}ms total, "
    ...
    print(f"• Pickle overhead increases with object complexity")
    print(f"• Concurrent requests show good scalability")
